...
class Experiment:
    ...
    def build_graph(self):
        edge_index = torch.tensor([[x[0] for x in d.kg_data], [
                                  x[2] for x in d.kg_data]], dtype=torch.long, device=device)
        edge_type = torch.tensor(
            [x[1] for x in d.kg_data], dtype=torch.int, device=device)
        g = geoData(edge_index=edge_index, edge_type=edge_type)

        # trainkg
        train_edge_index = torch.tensor([[x[0] for x in d.trainkg_data], [
                                        x[2] for x in d.trainkg_data]], dtype=torch.long, device=device)
        train_edge_type = torch.tensor(
            [x[1] for x in d.trainkg_data], dtype=torch.int, device=device)
        g_train = geoData(edge_index=train_edge_index,
                          edge_type=train_edge_type)

        # samplekg
        sample_edge_index = torch.tensor([[x[0] for x in d.samplekg_data], [
                                         x[2] for x in d.samplekg_data]], dtype=torch.long, device=device)
        sample_edge_type = torch.tensor(
            [x[1] for x in d.samplekg_data], dtype=torch.int, device=device)
        g_samp = geoData(edge_index=sample_edge_index,
                         edge_type=sample_edge_type)

        return g, g_train, g_samp

    def get_batch(self, train_data, trainids, idx):
        batch = train_data[idx:idx + self.batch_size]
        out = torch.tensor(batch, dtype=torch.float,
                           device=device)  # bs*nreg*nhour*2
        out = out[:, trainids, :, :]
        return out
    ...
    def train_and_eval(self):
        ...
        cond_pred_model = DeterministicFeedForwardNeuralNetwork(
            dim_in=dim_in, dim_out=dim_out).to(device)
        opt_nn = torch.optim.Adam(
            cond_pred_model.parameters(), lr=self.kwargs['nn_lr'])
        # pretrain cond_pred_model
        rmse_train = self.evaluate_guidance_model(
            cond_pred_model, train_loader)
        rmse_test = self.evaluate_guidance_model(cond_pred_model, test_loader)

        print(rmse_train)
        for it in tqdm(range(1, 1 + self.kwargs['pretrain_epochs'])):
            loss_epoch = self.train_guidance_model(
                cond_pred_model, train_loader, opt_nn)
            rmse_train = self.evaluate_guidance_model(
                cond_pred_model, train_loader)
            rmse_test = self.evaluate_guidance_model(
                cond_pred_model, test_loader)
        ...
        for it in range(1, self.num_iterations + 1):
            ...
            if it in [20, 40, 60, 80, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1500, 2000]:
                sampled_flow = diffusion.sample(
                    sampids, batch_size=self.kwargs['sample_num'])
                np.savez(archive_path + 'sample_{}.npz'.format(it),
                         sample=sampled_flow.detach().cpu().numpy())

            if it in [200, 500, 1000, 1500, 2000]:
                # save model
                torch.save(diffusion.state_dict(), archive_path +
                           "model_{}.pth".format(it))


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--num_iterations", type=int,
                        default=50000, nargs="?", help="Number of iterations.")
    ...
    parser.add_argument("--dim", type=int, default=64,
                        nargs="?", help="sin emb dim")
    parser.add_argument("--num_heads", type=int, default=2, nargs="?", help="")
    parser.add_argument("--num_rgcns", type=int, default=1, nargs="?", help="")
    parser.add_argument("--num_flowrgcns", type=int,
                        default=1, nargs="?", help="")

    parser.add_argument("--num_sas", type=int, default=1, nargs="?", help="")
    parser.add_argument("--dropout", type=float,
                        default=0.0, nargs="?", help="")
    parser.add_argument("--kge_cat_dim", type=int,
                        default=16, nargs="?", help="kge cat dim")
    parser.add_argument("--xt_cat_dim", type=int,
                        default=16, nargs="?", help="xt cat dim")
    parser.add_argument("--event_emb_dim", type=int,
                        default=16, nargs="?", help="event embedding dim")

    parser.add_argument('--pretrain', default=1, type=int,
                        help='1-use pretrain kg embedding')
    parser.add_argument('--freeze', default=1, type=int,
                        help='pretrain kg embedding freeze or not')

    parser.add_argument('--n_layer', default=5, type=int,
                        help='number of residual layers')

    parser.add_argument("--dataset", type=str,
                        default='bj', nargs="?", help="")
    # diffusion params
    parser.add_argument("--objective", type=str,
                        default='pred_noise', nargs="?", help="pred_noise/pred_x0")
    parser.add_argument("--loss_type", type=str,
                        default='l1', nargs="?", help="l1/l2")
    parser.add_argument("--beta_schedule", type=str,
                        default='cosine', nargs="?", help="cosine/linear")
    parser.add_argument("--diffusion_dteps", type=int,
                        default=1000, nargs="?", help="rt")
    parser.add_argument("--sample_num", type=int, default=4,
                        nargs="?", help="Number of samples")
    # condition prediction model params
    parser.add_argument("--nn_lr", type=float, default=0.001, nargs="?",
                        help="Learning rate of condition prediction model.")
    parser.add_argument("--pretrain_epochs", type=int, default=100,
                        nargs="?", help="Number of pretrain iterations.")
    parser.add_argument("--train_guidance_every_epochs", type=int,
                        default=1, nargs="?", help="train guidance every k epochs")
