...
class Data:
    def __init__(self, data_dir, dataset_name=None):
        self.reg2id, self.trainids, self.sampids, self.trainregs, self.sampleregs, self.regfeas = self.load_reg(
            data_dir)
        self.nreg = len(self.reg2id)
        self.ent2id, self.rel2id, self.kg_data, self.train_ent2id, self.trainkg_data, self.sample_ent2id, self.samplekg_data = self.load_kg(
            data_dir)
        self.train_data, self.min_data, self.max_data = self.load_flow(
            data_dir)  # ndays*nreg*nhour*2
        self.features, self.scale, self.scale_pred_data, self.scale_pred_X, self.KGE_pretrain = self.load_pretrain(
            data_dir)

        # 加载事件张量数据 E ∈ R^{N_l × T × d_event}
        self.event_tensor = self.load_events(data_dir, dataset_name)

        print('number of node=%d, number of edge=%d, number of relations=%d' %
              (len(self.ent2id), len(self.kg_data), len(self.rel2id)))
        print('region num={}'.format(len(self.reg2id)))
        print('train data={}'.format(len(self.train_data)))
        if self.event_tensor is not None:
            print('event tensor shape={}'.format(self.event_tensor.shape))
        print('load finished..')
    ...
    def load_kg(self, data_dir):
        ...
        ents = sorted(
            list(set([x[0] for x in kg_data_str] + [x[2] for x in kg_data_str])))
        ...
        kg_data = [[ent2id[x[0]], rel2id[x[1]], ent2id[x[2]]]
                   for x in kg_data_str]

        # train kg
        train_ent2id = dict([(x, i) for i, x in enumerate(self.trainregs)])
        trainkg_data = [[train_ent2id[x[0]], rel2id[x[1]],
                         train_ent2id[x[2]]] for x in trainkg_str]
        # sample kg
        sample_ent2id = dict([(x, i) for i, x in enumerate(self.sampleregs)])
        samplekg_data = [[sample_ent2id[x[0]], rel2id[x[1]],
                          sample_ent2id[x[2]]] for x in samplekg_str]
    ...
    def load_events(self, data_dir, dataset_name):
        ...
        Args:
            data_dir: 数据目录路径
            dataset_name: 数据集名称 (如 'HK', 'nyc', 'DC', 'BM')

        Returns:
            event_tensor: numpy数组，形状为 (nreg, T, d_event) 或 None（如果没有事件数据）
        """
        import os

        # 尝试加载事件数据
        event_files = [
            data_dir + 'Events/hk_events_2019.npz',  # HK数据集格式
            data_dir + '../events/hk_events_2019.npz',  # 备用路径
            data_dir + 'events.npz',  # 通用格式
        ]

        for event_file in event_files:
            if os.path.exists(event_file):
                print(f'Loading event tensor from: {event_file}')
                try:
                    event_data = np.load(event_file)
                    # nreg × T × d_event
                    event_tensor = event_data['event_tensor']

                    # 验证事件张量形状
                    if event_tensor.shape[0] != self.nreg:
                        print(
                            f'Warning: Event tensor regions ({event_tensor.shape[0]}) != data regions ({self.nreg})')
                        # 尝试对齐区域
                        # 如果事件数据区域数不同，可能需要重新映射

                    return event_tensor
                except Exception as e:
                    print(f'Error loading event tensor from {event_file}: {e}')
                    continue

        # 如果没有找到事件数据，返回None（使用零事件张量）
        print('Warning: No event tensor found. Using zero event features.')
        return None
