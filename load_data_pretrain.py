...
class Data:
    def __init__(self, data_dir, dataset_name=None):
        # 根据数据集名称调整数据路径
        if dataset_name == 'HK':
            # HK数据在HK_regions子文件夹中
            actual_data_dir = data_dir + 'HK_regions/'
        else:
            actual_data_dir = data_dir

        self.reg2id = self.load_reg(actual_data_dir)
        self.ent2id, self.rel2id, self.kg_data = self.load_kg(actual_data_dir)
